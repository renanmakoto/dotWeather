const WEATHER_VARIANTS = [
  ...
  },
  {
    ...
    dayGradient: ['#6c7a89', '#95a5a6'],
    nightGradient: ['#1f2a36', '#344550'],
    accent: '#c8d6e5',
    effect: 'drizzle',
  },
  {
    ...
    iconNight: 'rainy',
    dayGradient: ['#2c3e50', '#4ca1af'],
    nightGradient: ['#0f2027', '#2c5364'],
    accent: '#9ac7dc',
    effect: 'rain',
  },
  {
...
const CARD_GRADIENTS = {
  day: ['rgba(255,255,255,0.32)', 'rgba(255,255,255,0.08)'],
  night: ['rgba(21,33,61,0.72)', 'rgba(11,17,32,0.58)'],
}

const NIGHT_ACCENT = '#f0f6ff'

const DEFAULT_NIGHT_HOURS = { start: 19, end: 6 }
...
function isNightTime(timeString, sunTimes = {}) {
  ...
  const isSunriseValid = sunrise && !Number.isNaN(sunrise.getTime())
  const isSunsetValid = sunset && !Number.isNaN(sunset.getTime())

  if (isSunriseValid && isSunsetValid && sunrise < sunset) {
    return current < sunrise || current >= sunset
  }

  const hour = current.getHours()
  return hour >= DEFAULT_NIGHT_HOURS.start || hour < DEFAULT_NIGHT_HOURS.end
}
...
export function getWeatherPresentation(weatherCode, timeString, sunTimes) {
  ...
  const cardGradient = nightMode ? CARD_GRADIENTS.night : CARD_GRADIENTS.day
  const accent = nightMode ? NIGHT_ACCENT : variant.accent
